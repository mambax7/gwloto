...
$GLOBALS['xoopsOption']['template_main'] = 'gwloto_editmedia.tpl';
...
if (isset($_POST['mid'])) {
    $currentmedia = (int)$_POST['mid'];
} else if (isset($_GET['mid'])) {
    $currentmedia = (int)$_GET['mid'];
}
...
if (isset($places['currentauth'][_GWLOTO_USERAUTH_MD_VIEW])) {
    $user_can_view = true;
}
if ($user_can_edit) {
    $user_can_view = true;
...
if ($result) {
    ...
    if ($media_lang_fileref != 0) {
        $media_file_id = $media_lang_fileref;
    }
...
if ($op === 'deletetran') {
    if ($language_id == $language) {
        ...
        $err_message = _MD_GWLOTO_MEDIA_DELETE_DB_ERROR;
    }
    $op = 'display';
}
...
if ($op !== 'lchange') {
    if ($op !== 'updatetran') {
        ...
        if (isset($_POST['pid'])) {
            $media_auth_place = (int)$_POST['pid'];
        }
    }
...
if ($op !== 'display') {
    ...
    switch ($action_on_media_file) {
        ...
        case 'addlanguage':
            ...
            if (!$result) {
                ...
                $media_lang_fileref = $xoopsDB->getInsertId();
            }
            break;
    ...
    if (!$dberr) {
        ...
        if ($uploaded_stored_file != '') {
            unlink($uploaded_stored_file);
        }
        rollbackTransaction();
